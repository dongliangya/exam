...
public class ExamPaperServiceImpl extends BaseServiceImpl<ExamPaper> implements ExamPaperService {
    ...
    public ExamPaperEditRequestVM examPaperToVM1(Integer id,User user) {
        ExamPaper examPaper = examPaperMapper.selectByPrimaryKey(id);
        ExamPaperEditRequestVM vm = modelMapper.map(examPaper, ExamPaperEditRequestVM.class);
        vm.setLevel(examPaper.getGradeLevel());
        TextContent frameTextContent = textContentService.selectById(examPaper.getFrameTextContentId());
        ...
        if (user.getRole() == 1) {
            ArrayList<ExamPaperTitleItemVM> vms = new ArrayList<>();
            Random random = new Random();
            int size = examPaperTitleItemVMS.size();
            HashSet<Integer> integers = new HashSet<>();
            while (integers.size() < 2) {
                int randomSize = random.nextInt(size);
                integers.add(randomSize);
            }
            List<Integer> collect = integers.stream().collect(Collectors.toList());
            for (Integer integer : collect) {
                ExamPaperTitleItemVM itemVM = examPaperTitleItemVMS.get(integer);
                vms.add(itemVM);
            }
            vm.setTitleItems(vms);
            vm.setScore(ExamUtil.scoreToVM(examPaper.getScore()));
        } else {
            vm.setTitleItems(examPaperTitleItemVMS);
            vm.setScore(ExamUtil.scoreToVM(examPaper.getScore()));
